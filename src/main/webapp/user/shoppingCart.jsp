...
<!--文件体开始-->
		...
		<c:forEach items="${shoppingCart.lines }" var="line" varStatus="index">
        <tr>
			<form method="post" action="user/updateProduct" name="f1">
			<input type="hidden" name="bookid" value="${line.value.book.id}">
			<td class=tablebody2 valign=middle align=center width="">${index.index+1 }</td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="user/productDetail.jsp?productid=2">${line.value.book.name}</a></td>
			...
			<td class=tablebody2 valign=middle align=left width="">&nbsp;&nbsp;￥${line.value.book.price*line.value.num}</td>
			<td class=tablebody1 valign=middle align=center width="">
			<input type="button" value="取消" onclick="javascript:window.location='user/delOrderline?bookid=${line.value.book.id}';"> <input type="submit" value="保存修改"></td>
			</form>
		</tr>
		</c:forEach>
		...
		<tr>
			...
			<input type="button" value="清空购物车" onclick="javascript:window.location='user/shopcartClear';"></td>
		</tr>
                </table><br>
<!--文件尾开始-->
